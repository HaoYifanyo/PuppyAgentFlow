...
export const WorkflowDashboardModal: React.FC<WorkflowDashboardModalProps> = ({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      <Modal.Container width="w-[600px]">
        <Modal.Header title={<>📂 My Workflows</>} onClose={onClose} />

        <Modal.Body className="min-h-[300px] max-h-[60vh] overflow-y-auto">
          {loading && (
          ...
          {!loading && workflows.map((wf) => (
            ...
            >
              <div className="flex-1">
                <div className="font-bold text-sm text-stone-800 group-hover:text-rose-600 transition-colors">
                  {wf.name}
                </div>
