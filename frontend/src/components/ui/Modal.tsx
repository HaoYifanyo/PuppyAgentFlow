...
Modal.Header = ({ title, icon, onClose, closeTestId }: { title: ReactNode; icon?: ReactNode; onClose?: () => void; closeTestId?: string }) => (
  <div className="flex justify-between items-center p-4 border-b border-rose-100 bg-rose-50/50">
    ...
    {onClose && (
      <button onClick={onClose} data-testid={closeTestId} className="text-stone-400 hover:text-rose-600 transition-colors p-1 rounded-full hover:bg-rose-100 cursor-pointer">
        <X className="w-5 h-5" />
      </button>
    )}
  </div>
